...
<th
    {{ $attributes->merge(['class' => 'px-6 glacial py-3 bg-gray-100 dark:bg-gray-900'])->only(['class', 'style']) }}
>
    @unless ($sortable)
    ...
    @else
        <button {{ $attributes->except(['class', 'style']) }} class="flex items-center space-x-1 text-xs font-medium leading-4 tracking-wider text-left text-blue-500 uppercase dark:text-blue-400 group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
